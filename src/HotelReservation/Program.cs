...
var reservationRepo = new InMemoryReservationRepository(billingCalculator);
...
var domain = new ReservationDomainService();
var reservationService = new ReservationService(reservationRepo, roomRepo, logger, domain);
...
var policyRegistry = new CancellationPolicyRegistry(new ICancellationPolicy[]
{
    new FlexiblePolicy(),
    new ModeratePolicy(),
    new StrictPolicy(),
...
var cancellationService = new CancellationService(policyRegistry);
...
var bobLine = InvoiceLineData.FromReservation(bobForBilling);
var invoice = invoiceGenerator.Generate(bobLine);
invoiceGenerator.PrintInvoice(invoice, bobLine);
Console.WriteLine();
...
var nonRefundableRes = new NonRefundableReservation
{
    Id = "NR-001", GuestName = "Test NonRefundable", TotalPrice = 200m
};
Console.WriteLine(
    $"[OK] Non-refundable: ICancellableReservation non applicable — " +
    $"remboursement théorique {nonRefundableRes.CalculateRefund():F2} EUR (pas d'appel à Cancel sur ce type)");
Console.WriteLine();
